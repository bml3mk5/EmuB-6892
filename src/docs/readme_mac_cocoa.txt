...
------------------------------------------------------------------------------
 ...
 【注意】システムフォルダやアプリケーションフォルダにはコピーしないでください。
  ...
  2. 実機から取り出したROMイメージをbml3mk5.appフォルダがある場所、または
     適当なフォルダに置いてください。
  ...
  5. FDDを使用する場合は、メニューのFDDタイプ(FDD Type)、または、option+Fで
    ...
    実行すると、bml3mk5.appフォルダがある場所に以下のファイルが作成されます。
      bml3mk5.ini ... 設定ファイル
      bml3mk5.log ... 実行ログ
...
○ キー操作説明
  ...
  ---------------------------------------- ----------------------------------
  操作(Control)
    パワーオン/オフ(PowerOn/Off)           option + F3
    モードスイッチ(MODE Switch)            option + M
    リセットスイッチ(Reset Switch)         option + R
    FDDタイプモード切り替え                option + F
    ポーズ(Pause)                          option + Q
  ...
  FDD2
    挿入...(Insert...)                     option + F11
  FDD3
    挿入...(Insert...)                     option + F12
  画面(Screen)
    ...
    残像モード1(Afterimage1)               option + T (Afterimage2と切替)
    残像モード2(Afterimage2)               option + T (Afterimage1と切替)
    描画方法の切り替え                     option + Y
    フィルタ種類の切り替え                 option + U
  サウンド(Sound)
  ...
  オプション(Options)
    ...
    ジョイパッド使用(キー割当)(Enable Joypad(Key Assigned))
                                           option + J (PIA Typeと切替)
    ジョイパッド使用(PIA接続)(Enable Joypad(PIA Type))
                                           option + J (Key Assignedと切替)
    ライトペン有効(Enable Lightpen)        option + Ctrl
